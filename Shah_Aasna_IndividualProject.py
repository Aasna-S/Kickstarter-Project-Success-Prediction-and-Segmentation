# -*- coding: utf-8 -*-
"""
Created on Fri Nov 10 18:20:17 2023
...
# Import Grading Data
kickstarter_grading_df = pd.read_excel(r"C:\Users\Aasna\Downloads\Kickstarter-Grading-Sample.xlsx") 

# Pre-Process Grading Data
# Filtering the dataset for projects that are either "successful" or "failed"
filtered_kickstarter_g_data = kickstarter_grading_df[kickstarter_grading_df['state'].isin(['successful', 'failed'])]
...
data_selected = filtered_kickstarter_g_data[relevant_columns].copy()
...
encoded_weekdays = encoder.fit_transform(data_selected[weekday_columns])
encoded_weekdays_df = pd.DataFrame(encoded_weekdays, columns=encoder.get_feature_names_out(weekday_columns))
data_encoded_test = data_selected.drop(weekday_columns, axis=1)

# Encode Categorical Variables
categorical_columns = ['country', 'currency', 'category']
one_hot_encoder = OneHotEncoder(sparse=False)
encoded_categorical = one_hot_encoder.fit_transform(data_encoded_test[categorical_columns])
encoded_categorical_df = pd.DataFrame(encoded_categorical, columns=one_hot_encoder.get_feature_names_out(categorical_columns))
...
data_encoded_test = pd.concat([data_encoded_test.drop(categorical_columns, axis=1), encoded_weekdays_df, encoded_categorical_df], axis=1)

# Scale Numerical Variables
scaler = StandardScaler()
data_encoded_test[['goal', 'static_usd_rate']] = scaler.fit_transform(data_encoded_test[['goal', 'static_usd_rate']])

# Encoding the target variable
target = filtered_kickstarter_g_data['state'].replace({'successful': 1, 'failed': 0})

# Check if the lengths match
assert len(data_encoded_test) == len(target)

###########
#####ensure both test and training sets have the same columns and features (this part can be commented out if not needed)
#Get missing columns in the test set
missing_cols_in_test = set(data_encoded_train.columns) - set(data_encoded_test.columns)

#Add missing columns in test set with default value of 0
for col in missing_cols_in_test:
    data_encoded_test[col] = 0



# Get missing columns in the training set
missing_cols_in_train = set(data_encoded_test.columns) - set(data_encoded_train.columns)

# Add missing columns in training set with default value of 0
for col in missing_cols_in_train:
    data_encoded_train[col] = 0


# Ensure the order of columns in both datasets is the same
data_encoded_train = data_encoded_train.reindex(sorted(data_encoded_train.columns), axis=1)
data_encoded_test = data_encoded_test.reindex(sorted(data_encoded_test.columns), axis=1)


##########
# Setup the variables
X_grading = data_encoded_test
y_grading = target 


# Apply the model previously trained to the grading data
y_grading_pred = rf_classifier_best.predict(X_grading)
